...
public class UserDBRepository implements UserRepository {
    ...
    public User add(User entity) {
        ...
        try(PreparedStatement preparedStatement=conn.prepareStatement("insert into user(nume_u,prenume_u) values (?,?)", Statement.RETURN_GENERATED_KEYS)){
    ...
    public User findOne(Integer id) {
        ...
        try(PreparedStatement preparedStatement=conn.prepareStatement("select * from user where cod_u=?")) {
            ...
            if(resultSet.next()){
                user=new User(resultSet.getInt("cod_u"),resultSet.getString("nume_u"),resultSet.getString("prenume_u"));
            }
        } catch (SQLException e) {
    ...
    public List<User> findAll() {
        ...
        try(PreparedStatement preparedStatement=conn.prepareStatement("select * from user")){
            ...
            while(resultSet.next()){
                User user=new User(resultSet.getInt("cod_u"),resultSet.getString("nume_u"),resultSet.getString("prenume_u"));
                users.add(user);
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
